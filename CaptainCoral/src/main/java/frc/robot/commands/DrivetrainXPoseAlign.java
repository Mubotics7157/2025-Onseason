...
public class DrivetrainXPoseAlign extends Command {
    ...
    public double getLRSpeed() { 
        Pose3d currentPose = visionManager.getPose();

        if (currentPose == null) {
            return 0.0;
        } else {
            return visionManager.deriveLRPose() * SmartDashboard.getNumber("DrivePose Horizontal kP", Constants.Drivetrain_X_Pose_kP) * TunerConstants.kSpeedAt12Volts.in(MetersPerSecond);
        }
    }
